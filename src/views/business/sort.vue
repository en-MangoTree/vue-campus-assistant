<template>
    <div>
        ...
        <el-drawer
            ...
            direction="rtl"
            :before-close="cancel">
            <el-form :model="sortForm" ref="sortForm" :rules="formRules" label-width='120px' style="margin-right:20px">
                <el-form-item label="分类标签" prop="label" >
                    <el-input v-model="sortForm.label" />
                </el-form-item>
                <el-form-item label="标签值" prop="code">
                    <el-input v-model="sortForm.code" />
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        findPage() {
             this.loading = true;
            this.$ajax.post('/sort/findPage', this.queryInfo).then((res) => {
                 this.loading = false;
        ...
        deleteById(id) {
            ...
            type: "warning",
                ...
                this.$ajax.delete(`/sort/delete/${id}`).then((res) => {
        ...
        sure() {
            this.$refs.sortForm.validate((valid) => {
                ...
                if(this.sortForm.id === undefined || this.sortForm.id === null) {
                    this.$ajax.post('/sort/insert', this.sortForm).then((res) => {
                        this.$message.success(res.data.message);
                        this.open = false;
                        this.findPage();
                    });
                } else {
                    this.$ajax.post('/sort/update', this.sortForm).then((res) => {
                        this.$message.success(res.data.message);
                        this.open = false;
                        this.findPage();
                    });
        ...
        updateStatus(row) {
            this.$ajax.post('/sort/update', row).then((res) => {
